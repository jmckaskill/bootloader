...
struct eth_buffer {
	// 2 bytes of padding aligns the IP header to be at a 4 byte boundary
	uint8_t padding[2];
	uint8_t payload[0x5F0 - 2];
};
...
void send_eth_frame(void *data, int sz) {
	...
	struct eth_buffer *b = container_of(data, struct eth_buffer, payload);

	d->next = NULL;
	d->buffer = b;
	d->offset_buf_len = HW_ETH_OFFSET(sizeof(b->padding)) | sz;
...
void init_ethernet(uint8_t *mac) {
    ...
    write_little_32(mac, HW_CONTROL->mac[0].hi);
    write_little_16(mac+4, HW_CONTROL->mac[0].lo);
	...
	HW_CPSW_CPDMA->RX_BUFFER_OFFSET = offsetof(struct eth_buffer, payload);
	reset_rx_queue();
...
void process_eth_rx_interrupt() {
	...
	for (;;) {
		...
		if ((flags & (HW_ETH_FULL_PKT | HW_ETH_ERROR_MASK)) == (HW_ETH_FULL_PKT | HW_ETH_NO_ERROR)) {
			...
			process_eth_frame(b->payload, sz);
		}
