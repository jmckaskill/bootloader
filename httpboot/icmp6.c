...
void send_neighbor_advertisement() {
	struct ip6_header *ip = new_ip6_frame(&g_all_nodes_mac, &g_all_nodes, IP6_ICMP);
	if (ip) {
		...
		send_ip6_frame(ip, sz);
	}
}

static void send_echo_reply(struct ip6_header *ip, uint32_t idseq, const void *data, int sz) {
	struct ip6_header *rep = new_ip6_frame(&ip->eth_src, &ip->ip6_src, IP6_ICMP);
	if (rep) {
